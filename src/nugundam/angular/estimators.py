"""Angular estimator formulas and uncertainty propagation helpers."""
from __future__ import annotations

import numpy as np

from .models import AngularAutoCounts, AngularCrossCounts, AngularCorrelationResult
...
def _normalize_auto_unweighted(counts: AngularAutoCounts):
    """
...
def _normalize_auto_weighted(counts: AngularAutoCounts, data_weights: np.ndarray):
    """
    ...
    if counts.dd_boot is not None and counts.dd_boot.size > 0:
        ...
        for j in range(counts.dd_boot.shape[1]):
            nrm = counts.norm_dd_boot[j] if counts.norm_dd_boot is not None else norm_DD
            if nrm > 0:
...
def compute_auto_wtheta(counts: AngularAutoCounts, *, estimator: str, data_weights: np.ndarray | None = None, sum_w_data: float | None = None, sum_w2_data: float | None = None) -> np.ndarray:
    """Reconstruct the angular auto-correlation field from stored counts."""
    if data_weights is None and sum_w_data is None:
        ...
        rr_n = counts.rr / norm_RR if counts.rr is not None else None
        dr_n = counts.dr / norm_DR if counts.dr is not None and norm_DR > 0 else None
    xi = np.zeros_like(counts.dd, dtype=np.float64)
    for i in range(len(xi)):
        if estimator == "NAT":
            if rr_n is not None and rr_n[i] > 0:
                xi[i] = dd_n[i] / rr_n[i] - 1.0
        elif estimator == "DP":
            if dr_n is not None and dr_n[i] > 0:
                xi[i] = dd_n[i] / dr_n[i] - 1.0
        elif estimator == "LS":
            if rr_n is not None and rr_n[i] > 0 and dr_n is not None:
                xi[i] = (dd_n[i] - 2.0 * dr_n[i] + rr_n[i]) / rr_n[i]
        else:
            raise ValueError(f"Unsupported auto estimator: {estimator}")
    return xi


def compute_cross_wtheta(counts: AngularCrossCounts, *, estimator: str, sum_w1: float | None = None, sum_w2: float | None = None) -> np.ndarray:
    """Reconstruct the angular cross-correlation field from stored counts."""
    ...
    xi = np.zeros_like(counts.d1d2, dtype=np.float64)
    for i in range(len(xi)):
        if estimator == "NAT":
            if r1r2_n is not None and r1r2_n[i] > 0:
                xi[i] = d1d2_n[i] / r1r2_n[i] - 1.0
        elif estimator == "DP":
            if d1r2_n is not None and d1r2_n[i] > 0:
                xi[i] = d1d2_n[i] / d1r2_n[i] - 1.0
        elif estimator == "LS":
            if r1r2_n is not None and r1r2_n[i] > 0 and d1r2_n is not None and r1d2_n is not None:
                xi[i] = (d1d2_n[i] - d1r2_n[i] - r1d2_n[i] + r1r2_n[i]) / r1r2_n[i]
        else:
            raise ValueError(f"Unsupported cross estimator: {estimator}")
    return xi

def estimate_auto(counts: AngularAutoCounts, *, estimator: str, data_weights: np.ndarray | None = None):
    """
    ...
    xi = np.zeros_like(counts.dd, dtype=np.float64)
    xierr = np.zeros_like(counts.dd, dtype=np.float64)
    for i in range(len(xi)):
        if estimator == "NAT":
            if rr_n is not None and rr_n[i] > 0:
                xi[i] = dd_n[i] / rr_n[i] - 1.0
                if bdd_n is not None:
                    xierr[i] = _boot_std(bdd_n[i] / rr_n[i] - 1.0)
        elif estimator == "DP":
            if dr_n is not None and dr_n[i] > 0:
                xi[i] = dd_n[i] / dr_n[i] - 1.0
                if bdd_n is not None:
                    xierr[i] = _boot_std(bdd_n[i] / dr_n[i] - 1.0)
        elif estimator == "LS":
            if rr_n is not None and rr_n[i] > 0 and dr_n is not None:
                xi[i] = (dd_n[i] - 2.0 * dr_n[i] + rr_n[i]) / rr_n[i]
                if bdd_n is not None:
                    xierr[i] = _boot_std((bdd_n[i] - 2.0 * dr_n[i] + rr_n[i]) / rr_n[i])
        else:
            raise ValueError(f"Unsupported auto estimator: {estimator}")
    return AngularCorrelationResult(theta_edges=counts.theta_edges, theta_centers=counts.theta_centers, wtheta=xi, wtheta_err=xierr, estimator=estimator, counts=counts, metadata={"weighted": data_weights is not None})


def estimate_cross(counts: AngularCrossCounts, *, estimator: str, sum_w1: float | None = None, sum_w2: float | None = None):
    """
    ...
    r1r2_n = counts.r1r2 / (nr1 * nr2) if counts.r1r2 is not None else None
    xi = np.zeros_like(counts.d1d2, dtype=np.float64)
    xierr = np.zeros_like(counts.d1d2, dtype=np.float64)
        ...
        if estimator == "NAT":
            if r1r2_n is not None and r1r2_n[i] > 0:
                ...
                if b_d1d2_n is not None:
                    xierr[i] = _boot_std(b_d1d2_n[i] / r1r2_n[i] - 1.0)
        elif estimator == "DP":
            if d1r2_n is not None and d1r2_n[i] > 0:
                xi[i] = d1d2_n[i] / d1r2_n[i] - 1.0
                if b_d1d2_n is not None and b_d1r2_n is not None:
                    xierr[i] = _boot_std(b_d1d2_n[i] / b_d1r2_n[i] - 1.0)
        elif estimator == "LS":
            if r1r2_n is not None and r1r2_n[i] > 0 and d1r2_n is not None and r1d2_n is not None:
                xi[i] = (d1d2_n[i] - d1r2_n[i] - r1d2_n[i] + r1r2_n[i]) / r1r2_n[i]
                if b_d1d2_n is not None and b_d1r2_n is not None:
                    xierr[i] = _boot_std((b_d1d2_n[i] - b_d1r2_n[i] - r1d2_n[i] + r1r2_n[i]) / r1r2_n[i])
        else:
            raise ValueError(f"Unsupported cross estimator: {estimator}")
    return AngularCorrelationResult(theta_edges=counts.theta_edges, theta_centers=counts.theta_centers, wtheta=xi, wtheta_err=xierr, estimator=estimator, counts=counts, metadata={"weighted": sum_w1 is not None or sum_w2 is not None})
