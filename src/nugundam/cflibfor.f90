  ...
  !
  !   * compute (auto)counts angular space
  !   * compute (cross)counts in angular space
  ...
  ! Contact Author
  !===============
  ! Emilio Donoso (emiliodon@gmail.com)
...
implicit none
character(len=*), intent(in) :: cntid, progressf
integer, intent(in) :: nstrip
integer :: pu

if(len_trim(progressf)>0) then
   open(newunit=pu,file=trim(progressf),action='write',position='append',status='unknown')
   write(pu,*) ' '
   write(pu,'(a,a,a,i0,a)') '====  Counting ',trim(cntid),' pairs in ', nstrip, ' DEC strips  ===='
   flush(pu)
   close(pu)
else
   write(*,*) ' '
   write(*,'(a,a,a,i0,a)') '====  Counting ',trim(cntid),' pairs in ', nstrip, ' DEC strips  ===='
   flush(6)
endif
end subroutine write_progress_header

subroutine write_progress_stripe(cntid,istrip,nstrip,progressf)
implicit none
character(len=*), intent(in) :: cntid, progressf
integer, intent(in) :: istrip, nstrip
integer :: pu

if(len_trim(progressf)>0) then
   open(newunit=pu,file=trim(progressf),action='write',position='append',status='unknown')
   write(pu,'(a,a,a,i0,a,i0)') '[',trim(cntid),'] stripe ',istrip,'/',nstrip
   flush(pu)
   close(pu)
else
   write(*,'(a,a,a,i0,a,i0)') '[',trim(cntid),'] stripe ',istrip,'/',nstrip
   flush(6)
endif
end subroutine write_progress_stripe

subroutine write_progress_range(cntid,istrip,nstrip,p1,p2,progressf)
implicit none
character(len=*), intent(in) :: cntid, progressf
integer, intent(in) :: istrip, nstrip, p1, p2
integer :: pu

if(len_trim(progressf)>0) then
   open(newunit=pu,file=trim(progressf),action='write',position='append',status='unknown')
   write(pu,'(a,a,a,i0,a,i0,a,i0,a,i0)') '[',trim(cntid),'] stripe ',istrip,'/',nstrip,'  particles ',p1,'-',p2
   flush(pu)
   close(pu)
else
   write(*,'(a,a,a,i0,a,i0,a,i0,a,i0)') '[',trim(cntid),'] stripe ',istrip,'/',nstrip,'  particles ',p1,'-',p2
   flush(6)
endif
end subroutine write_progress_range
...
subroutine th_A(nt,npt,dec,x,y,z,nsep,sep,sbound,mxh1,mxh2,cntid,logf,progressf,sk,ll,aa)
!===============================================================================
! NAME
!  th_A()
!
...
aa      = 0.d0
dltdec  = sep(nsep+1)
sep2    = 4.0d0*(sin(0.5*sep*deg2rad))**2
sep2max = sep2(nsep+1)
...
subroutine th_A_wg(nt,npt,dec,wei,x,y,z,nsep,sep,sbound,mxh1,mxh2,wfib,cntid,logf,progressf,sk,ll,aa)
!===============================================================================
! NAME
!  th_A_wg()
!
...
subroutine th_Ab_wg(nt,npt,dec,wei,x,y,z,nsep,sep,sbound,mxh1,mxh2,nbts,bseed,wfib, &
                    cntid,logf,progressf,sk,ll,aa,baa,normb,sumwb)
!===============================================================================
! NAME
!  th_Ab_wg()
!
...
real(kind=8) :: x(npt),y(npt),z(npt),sep(nsep+1),sep2(nsep+1),sep2max,sbound(4),xi,yi,zi
real(kind=4) :: wei(npt),wi,wpp
real(kind=8) :: aa(nsep),baa(nbts,nsep),normb(nbts),sumwb(nbts)
real(kind=4) :: wbts(nbts,npt)
real(kind=8) :: wb,sb,tmpw
integer      :: ib
...
! Generate bootstrap samples
call bootstrap(npt,nbts,bseed,wbts)
! Compute per-bootstrap total-weight normalizations for DD
! Each bootstrap resample has multiplicities wbts(ib,i) for each object i.
! For weighted DD, the effective object weight is w'_i = wei(i) * wbts(ib,i),
! and the total weighted number of unique pairs is:
!   normb(ib) = 0.5 * [ (sum_i w'_i)^2 - sum_i (w'_i)^2 ]
normb = 0.0d0
sumwb = 0.0d0
do ib = 1, nbts
    wb = 0.0d0
    sb = 0.0d0
    do i = 1, npt
        tmpw = dble(wei(i)) * dble(wbts(ib,i))
        wb = wb + tmpw
        sb = sb + tmpw*tmpw
    end do
    normb(ib) = 0.5d0 * (wb*wb - sb)
    sumwb(ib) = wb
end do
...
subroutine th_C(nt,npt,ra,dec,x,y,z,npt1,x1,y1,z1, &
                nsep,sep,sbound,mxh1,mxh2,cntid,logf,progressf,sk1,ll1,cdth)
!===============================================================================
! NAME
!  th_C()
!
...
!----------------------------------------------------
! Some inits
dpart = (npt + mxh1 - 1)/mxh1 ! ceil(npt/mxh1): keep progress messages <= mxh1
stm2  = sin(dltdec*0.5*deg2rad)
jq1m  = int(dltdec/hc1)+1
...
do i=1,npt
   ...
   nadv = (i-1)/dpart + 1
   p1   = (nadv-1)*dpart + 1
   p2   = min(nadv*dpart,npt)
   if(i==p1) then
       !note we are not really counting in mxh1 strips, just mimicking
       !$omp critical
...
subroutine th_C_wg(nt,npt,ra,dec,wei,x,y,z,npt1,wei1,x1,y1,z1, &
                   nsep,sep,sbound,mxh1,mxh2,wfib,cntid,logf,progressf,sk1,ll1,cdth)
!===============================================================================
! NAME
!  th_C_wg()
!
...
subroutine th_Cb(nt,npt,ra,dec,x,y,z,npt1,x1,y1,z1, &
                 nsep,sep,sbound,mxh1,mxh2,nbts,bseed,cntid,logf,progressf,sk1,ll1,cdth,bcdth)
!===============================================================================
! NAME
!  th_Cb()
!
...
subroutine th_Cb_wg(nt,npt,ra,dec,wei,x,y,z,npt1,wei1,x1,y1,z1, &
                    nsep,sep,sbound,mxh1,mxh2,nbts,bseed,wfib,cntid,logf,progressf,sk1,ll1,cdth,bcdth)
!===============================================================================
! NAME
!  th_Cb_wg()
!
...
subroutine rppi_A(nt,npt,dec,dc,x,y,z,nsepp,sepp,nsepv,sepv,sbound, &
           mxh1,mxh2,mxh3,cntid,logf,progressf,sk,ll,aapv)
!===============================================================================
! NAME
!  rppi_A()
!
               ...
               lp_jq1: do jq1=jq1min,iq1+jq1m
                  ...
                  lp_jq2: do jq2=jq2min,jq2max
                     ...
                     do while(j/=0)
                        rv = abs(dci-dc(j))
                        if(rv<=rvmax) then
                           rp2 = dci*dc(j)*((xi-x(j))**2 + (yi-y(j))**2 + (zi-z(j))**2)
                           if(rp2<=rpmax2) then
...
subroutine rppi_A_wg(nt,npt,dec,dc,wei,x,y,z,nsepp,sepp,nsepv,sepv,sbound, &
           mxh1,mxh2,mxh3,wfib,cntid,logf,progressf,sk,ll,aapv)
!===============================================================================
! NAME
!  rppi_A_wg()
!
...
subroutine rppi_Ab_wg(nt,npt,dec,dc,wei,x,y,z,nsepp,sepp,nsepv,sepv,sbound, &
           mxh1,mxh2,mxh3,nbts,bseed,wfib,cntid,logf,progressf,sk,ll,aapv,baapv)
!===============================================================================
! NAME
!  rppi_Ab_wg()
!
...
subroutine rppi_C(nt,npt,ra,dec,dc,x,y,z,npt1,dc1,x1,y1,z1, &
           nsepp,sepp,nsepv,sepv,sbound,mxh1,mxh2,mxh3,cntid,logf,progressf,sk1,ll1,cdpv)
!===============================================================================
! NAME
!  rppi_C()
!
      ...
      lp_jq1: do jq1=iq1-jq1m,iq1+jq1m
         ...
         lp_jq2: do jq2=jq2min,jq2max
            ...
            do while(j/=0)
               ...
               rv  = abs(dci-dc1(j))
               if(rv<=rvmax) then
                  rp2 = dci*dc1(j)*((xi-x1(j))**2 + (y(i)-y1(j))**2 + (z(i)-z1(j))**2)
                  if(rp2<=rpmax2) then
                     jj = 1 + int(rv*idsepv)       !find rv bin number
                     if(rp2>sepp2(nsepp)) then 
...
subroutine rppi_C_wg(nt,npt,ra,dec,dc,wei,x,y,z,npt1,dc1,wei1,x1,y1,z1, &
           nsepp,sepp,nsepv,sepv,sbound,mxh1,mxh2,mxh3,wfib,cntid,logf,progressf,sk1,ll1,cdpv)
!===============================================================================
! NAME
!  rppi_C_wg()
!
...
subroutine rppi_Cb_wg(nt,npt,ra,dec,dc,wei,x,y,z,npt1,dc1,wei1,x1,y1,z1, &
           ...
           logf,progressf,sk1,ll1,cdpv,bcdpv)
!===============================================================================
! NAME
!  rppi_Cb_wg()
!
...
subroutine th_Ajk(nt,npt,dec,x,y,z,reg,nreg,nsep,sep,sbound,mxh1,mxh2,cntid,logf,progressf,sk,ll,aa,touch)
implicit none
real(kind=8) :: dec(npt),ral,rau,decl,decu,hc1,hc2,shth2,stm2,dltra,dltdec
real(kind=8) :: x(npt),y(npt),z(npt),sep(nsep+1),sep2(nsep+1),sep2max,sbound(4),xi,yi,zi
real(kind=8) :: aa(nsep),touch(nreg,nsep)
integer      :: reg(npt)
integer      :: nt,nthr,npt,mxh1,mxh2,nsep,sk(mxh2,mxh1),ll(npt),nc1,nc2,nreg
integer      :: jq1m,iq1,iq2,jq1,jq2,i,j,jq2max,jq2min,jq2m,jq2t,ii,regi,regj
character    :: cntid*32,logf*80,progressf*512
open(11,file=logf,action='write',access='append')
aa = 0.d0
touch = 0.d0
...
jq1m = int(dltdec/hc1)+1
call write_progress_header(cntid,mxh1,progressf)
!$omp parallel do reduction(+:aa,touch) default(shared) &
!$omp& private(iq1,iq2,jq1,jq2,dltra,jq2m,jq2min,jq2max,jq2t,j,i) &
!$omp& private(xi,yi,zi,shth2,ii,regi,regj) &
!$omp& schedule(guided) if(nthr>1)
do iq1=1,nc1
   !$omp critical
   call write_progress_stripe(cntid,iq1,mxh1,progressf)
            ...
            lp_jq2: do jq2=jq2min,jq2max
               ...
               do while(j/=0)
                  ...
                  if(shth2<=sep2max) then
                      ...
                      if(shth2>sep2(nsep)) then
                          aa(nsep) = aa(nsep) + 1.0d0
                          touch(regi,nsep) = touch(regi,nsep) + 1.0d0
                          if(regj/=regi) touch(regj,nsep) = touch(regj,nsep) + 1.0d0
                          goto 78
                      endif
                      if(shth2>sep2(nsep-1)) then
                          aa(nsep-1) = aa(nsep-1) + 1.0d0
                          touch(regi,nsep-1) = touch(regi,nsep-1) + 1.0d0
                          if(regj/=regi) touch(regj,nsep-1) = touch(regj,nsep-1) + 1.0d0
                          goto 78
                      endif
                      if(shth2>sep2(nsep-2)) then
                          aa(nsep-2) = aa(nsep-2) + 1.0d0
                          touch(regi,nsep-2) = touch(regi,nsep-2) + 1.0d0
                          if(regj/=regi) touch(regj,nsep-2) = touch(regj,nsep-2) + 1.0d0
                          goto 78
                      endif
                      if(shth2>sep2(nsep-3)) then
                          aa(nsep-3) = aa(nsep-3) + 1.0d0
                          touch(regi,nsep-3) = touch(regi,nsep-3) + 1.0d0
                          if(regj/=regi) touch(regj,nsep-3) = touch(regj,nsep-3) + 1.0d0
                          goto 78
                      endif
                      do ii=nsep-4,1,-1
                         if(shth2>sep2(ii)) then
                            aa(ii) = aa(ii) + 1.0d0
                            touch(regi,ii) = touch(regi,ii) + 1.0d0
                            if(regj/=regi) touch(regj,ii) = touch(regj,ii) + 1.0d0
                            goto 78
                         endif
                      enddo
                  endif
...
!$omp end parallel do
close(11)
end subroutine th_Ajk

subroutine th_Ajk_wg(nt,npt,dec,wei,x,y,z,reg,nreg,nsep,sep,sbound,mxh1,mxh2,wfib,cntid,logf,progressf,sk,ll,aa,touch)
implicit none
real(kind=8) :: dec(npt),ral,rau,decl,decu,hc1,hc2,shth2,stm2,dltra,dltdec
real(kind=8) :: x(npt),y(npt),z(npt),sep(nsep+1),sep2(nsep+1),sep2max,sbound(4),xi,yi,zi
real(kind=4) :: wei(npt),wi,wpp
real(kind=8) :: aa(nsep),touch(nreg,nsep)
integer      :: reg(npt)
integer      :: nt,nthr,npt,mxh1,mxh2,nsep,sk(mxh2,mxh1),ll(npt),nc1,nc2,nreg,wfib
integer      :: jq1m,iq1,iq2,jq1,jq2,i,j,jq2max,jq2min,jq2m,jq2t,ii,regi,regj
character    :: cntid*32,logf*80,progressf*512
open(11,file=logf,action='write',access='append')
aa = 0.d0
...
!$omp parallel do reduction(+:aa,touch) default(shared) &
!$omp& private(iq1,iq2,jq1,jq2,dltra,jq2m,jq2min,jq2max,jq2t,j,i) &
!$omp& private(xi,yi,zi,wi,wpp,shth2,ii,regi,regj) &
!$omp& schedule(guided) if(nthr>1)
do iq1=1,nc1
   !$omp critical
   call write_progress_stripe(cntid,iq1,mxh1,progressf)
      ...
      do while(i/=0)
         ...
         lp_jq1: do jq1=iq1,iq1+jq1m
            ...
            lp_jq2: do jq2=jq2min,jq2max
               ...
               xi = x(i); yi = y(i); zi = z(i); wi = wei(i)
               do while(j/=0)
                  shth2 = (xi-x(j))**2 + (yi-y(j))**2 + (zi-z(j))**2
                  if(shth2<=sep2max) then
                      wpp = wi*wei(j)
                      if(wfib==1) wpp = wpp*wfiber(shth2)
                      regj = reg(j)+1
                      if(shth2>sep2(nsep)) then
                          aa(nsep) = aa(nsep) + wpp
                          touch(regi,nsep) = touch(regi,nsep) + wpp
                          if(regj/=regi) touch(regj,nsep) = touch(regj,nsep) + wpp
                          goto 78
                      endif
                      if(shth2>sep2(nsep-1)) then
                          aa(nsep-1) = aa(nsep-1) + wpp
                          touch(regi,nsep-1) = touch(regi,nsep-1) + wpp
                          if(regj/=regi) touch(regj,nsep-1) = touch(regj,nsep-1) + wpp
                          goto 78
                      endif
                      if(shth2>sep2(nsep-2)) then
                          aa(nsep-2) = aa(nsep-2) + wpp
                          touch(regi,nsep-2) = touch(regi,nsep-2) + wpp
                          if(regj/=regi) touch(regj,nsep-2) = touch(regj,nsep-2) + wpp
                          goto 78
                      endif
                      if(shth2>sep2(nsep-3)) then
                          aa(nsep-3) = aa(nsep-3) + wpp
                          touch(regi,nsep-3) = touch(regi,nsep-3) + wpp
                          if(regj/=regi) touch(regj,nsep-3) = touch(regj,nsep-3) + wpp
                          goto 78
                      endif
                      do ii=nsep-4,1,-1
                         if(shth2>sep2(ii)) then
                            aa(ii) = aa(ii) + wpp
                            touch(regi,ii) = touch(regi,ii) + wpp
                            if(regj/=regi) touch(regj,ii) = touch(regj,ii) + wpp
                            goto 78
                         endif
                      enddo
                  endif
...
!$omp end parallel do
close(11)
end subroutine th_Ajk_wg

subroutine th_Cjk(nt,npt,ra,dec,x,y,z,reg,npt1,x1,y1,z1,reg1,nreg,nsep,sep,sbound,mxh1,mxh2,cntid,logf,progressf,sk1,ll1,cdth,touch)
implicit none
real(kind=8) :: ra(npt),dec(npt),sbound(4),ral,rau,decl,decu,hc1,hc2,shth2,stm2,dltra,dltdec
real(kind=8) :: x(npt),y(npt),z(npt),x1(npt1),y1(npt1),z1(npt1),sep(nsep+1),sep2(nsep+1),sep2max,xi,yi,zi
real(kind=8) :: cdth(nsep),touch(nreg,nsep)
integer      :: reg(npt),reg1(npt1)
integer      :: sk1(mxh2,mxh1),ll1(npt1),nt,nthr,npt,mxh1,mxh2,nsep,npt1,nreg
integer      :: fracp,dpart,nadv,nc1,nc2,jq1m,jq2m
integer      :: i,ii,j,iq1,iq2,jq1,jq2,jq2min,jq2max,jq2t,p1,p2,regi,regj
character    :: cntid*32,logf*80,progressf*512
open(11,file=logf,action='write',access='append')
...
call write_progress_header(cntid,mxh1,progressf)
!$omp parallel do reduction(+:cdth,touch) default(shared) &
!$omp& private(iq1,iq2,jq1,jq2,dltra,jq2m,jq2min,jq2max,jq2t,j,i) &
!$omp& private(xi,yi,zi,shth2,ii,p1,p2,regi,regj) &
!$omp& schedule(guided) firstprivate(fracp) if(nthr>1)
do i=1,npt
   xi  = x(i)
   yi  = y(i)
...
subroutine th_Cjk_wg(nt,npt,ra,dec,wei,x,y,z,reg,npt1,wei1,x1,y1,z1,reg1,nreg, &
           nsep,sep,sbound,mxh1,mxh2,wfib,cntid,logf,progressf,sk1,ll1,cdth,touch)
implicit none
real(kind=8) :: ra(npt),dec(npt),sbound(4),ral,rau,decl,decu,hc1,hc2,shth2,stm2,dltra,dltdec
real(kind=8) :: x(npt),y(npt),z(npt),x1(npt1),y1(npt1),z1(npt1),sep(nsep+1),sep2(nsep+1),sep2max,xi,yi,zi
real(kind=4) :: wei(npt),wei1(npt1),wi,wpp
real(kind=8) :: cdth(nsep),touch(nreg,nsep)
integer      :: reg(npt),reg1(npt1)
integer      :: sk1(mxh2,mxh1),ll1(npt1),nt,nthr,npt,mxh1,mxh2,nsep,npt1,nreg,wfib
integer      :: fracp,dpart,nadv,nc1,nc2,jq1m,jq2m
integer      :: i,ii,j,iq1,iq2,jq1,jq2,jq2min,jq2max,jq2t,p1,p2,regi,regj
character    :: cntid*32,logf*80,progressf*512
open(11,file=logf,action='write',access='append')
...
!$omp parallel do reduction(+:cdth,touch) default(shared) &
!$omp& private(iq1,iq2,jq1,jq2,dltra,jq2m,jq2min,jq2max,jq2t,j,i) &
!$omp& private(xi,yi,zi,wi,wpp,shth2,ii,p1,p2,regi,regj) &
!$omp& schedule(guided) firstprivate(fracp) if(nthr>1)
do i=1,npt
   xi  = x(i)
   yi  = y(i)
...
subroutine rppi_Ajk(nt,npt,dec,dc,x,y,z,reg,nreg,nsepp,sepp,nsepv,sepv,sbound, &
           mxh1,mxh2,mxh3,cntid,logf,progressf,sk,ll,aapv,touch)
implicit none
real(kind=8)  :: dec(npt),sbound(6),ral,rau,decl,decu,dcoml,dcomu,hc1,hc2,hc3,rp2
real(kind=8)  :: x(npt),y(npt),z(npt),dc(npt),rcl,stm2,dltdec,dltra,dci,xi,yi,zi
real(kind=8)  :: sepp(nsepp+1),sepv(nsepv+1),sepp2(nsepp+1),rpmax,rpmax2,rvmax,rv,idsepv
real(kind=8)  :: aapv(nsepv,nsepp),touch(nreg,nsepv,nsepp)
integer       :: reg(npt)
integer       :: sk(mxh3,mxh2,mxh1),ll(npt),mxh1,mxh2,mxh3,npt,nsepp,nsepv,nreg
integer       :: nt,nthr,nc1,nc2,nc3,jq1m,jq2m
integer       :: i,ii,j,jj,iq1,iq2,iq3,jq1,jq2,jq3,jq1min,jq2min,jq2max,jq2t
integer       :: regi,regj
character     :: cntid*32,logf*80,progressf*512
...
!$omp& private(iq1,iq2,iq3,jq1,jq2,jq3,rcl,stm2,dltdec,jq1m,jq1min) &
!$omp& private(dltra,jq2m,jq2min,jq2max,jq2t,j,i,xi,yi,zi,dci,rv,rp2,ii,jj,regi,regj) &
!$omp& schedule(dynamic) if(nthr>1)
do iq1=1,nc1
   !$omp critical
   call write_progress_stripe(cntid,iq1,mxh1,progressf)
                     ...
                     do while(j/=0)
                        ...
                        if(rv<=rvmax) then
                           ...
                           if(rp2<=rpmax2) then
                               ...
                               if(rp2>sepp2(nsepp)) then
                                  aapv(jj,nsepp) = aapv(jj,nsepp) + 1.0d0
                                  touch(regi,jj,nsepp) = touch(regi,jj,nsepp) + 1.0d0
                                  if(regj/=regi) touch(regj,jj,nsepp) = touch(regj,jj,nsepp) + 1.0d0
                                  goto 70
                               endif
                               do ii=nsepp-1,1,-1
                                  if(rp2>sepp2(ii)) then
                                     aapv(jj,ii) = aapv(jj,ii) + 1.0d0
                                     touch(regi,jj,ii) = touch(regi,jj,ii) + 1.0d0
                                     if(regj/=regi) touch(regj,jj,ii) = touch(regj,jj,ii) + 1.0d0
                                     goto 70
                                  endif
                               enddo
                           endif
...
subroutine rppi_Ajk_wg(nt,npt,dec,dc,wei,x,y,z,reg,nreg,nsepp,sepp,nsepv,sepv,sbound, &
           mxh1,mxh2,mxh3,wfib,cntid,logf,progressf,sk,ll,aapv,touch)
implicit none
real(kind=8)  :: dec(npt),sbound(6),ral,rau,decl,decu,dcoml,dcomu,hc1,hc2,hc3,rp2
real(kind=4)  :: wei(npt),wpp,wi
real(kind=8)  :: x(npt),y(npt),z(npt),dc(npt),rcl,stm2,dltdec,dltra,dci,xi,yi,zi
...
integer       :: reg(npt)
integer       :: sk(mxh3,mxh2,mxh1),ll(npt),mxh1,mxh2,mxh3,npt,nsepp,nsepv,nreg
integer       :: nt,nthr,nc1,nc2,nc3,jq1m,jq2m,wfib
integer       :: i,ii,j,jj,iq1,iq2,iq3,jq1,jq2,jq3,jq1min,jq2min,jq2max,jq2t
integer       :: regi,regj
character     :: cntid*32,logf*80,progressf*512
open(11,file=logf,action='write',access='append')
...
!$omp parallel do reduction(+:aapv,touch) default(shared) &
!$omp& private(iq1,iq2,iq3,jq1,jq2,jq3,rcl,stm2,dltdec,jq1m,jq1min) &
!$omp& private(dltra,jq2m,jq2min,jq2max,jq2t,j,i,xi,yi,zi,dci,rv,rp2,ii,jj,wi,wpp,shth2,regi,regj) &
!$omp& schedule(guided) if(nthr>1)
do iq1=1,nc1
   !$omp critical
   call write_progress_stripe(cntid,iq1,mxh1,progressf)
                     ...
                     do while(j/=0)
                        ...
                        if(rv<=rvmax) then
                           ...
                           if(rp2<=rpmax2) then
                               ...
                               if(rp2>sepp2(nsepp)) then
                                  aapv(jj,nsepp) = aapv(jj,nsepp) + wpp
                                  touch(regi,jj,nsepp) = touch(regi,jj,nsepp) + wpp
                                  if(regj/=regi) touch(regj,jj,nsepp) = touch(regj,jj,nsepp) + wpp
                                  goto 70
                               endif
                               do ii=nsepp-1,1,-1
                                  if(rp2>sepp2(ii)) then
                                     aapv(jj,ii) = aapv(jj,ii) + wpp
                                     touch(regi,jj,ii) = touch(regi,jj,ii) + wpp
                                     if(regj/=regi) touch(regj,jj,ii) = touch(regj,jj,ii) + wpp
                                     goto 70
                                  endif
                               enddo
                           endif
...
subroutine rppi_Cjk(nt,npt,ra,dec,dc,x,y,z,reg,npt1,dc1,x1,y1,z1,reg1, &
           nreg,nsepp,sepp,nsepv,sepv,sbound,mxh1,mxh2,mxh3,cntid,logf,progressf,sk1,ll1,cdpv,touch)
implicit none
real(kind=8)  :: ra(npt),dec(npt),sbound(6),ral,rau,decl,decu,dcoml,dcomu,hc1,hc2,hc3,rp2
real(kind=8)  :: x(npt),y(npt),z(npt),x1(npt1),y1(npt1),z1(npt1),rcl,stm2,dltdec,dltra
real(kind=8)  :: dc(npt),dc1(npt1),xi,dci
real(kind=8)  :: sepp(nsepp+1),sepv(nsepv+1),sepp2(nsepp+1),rpmax,rpmax2,rvmax,rv,idsepv
real(kind=8)  :: cdpv(nsepv,nsepp),touch(nreg,nsepv,nsepp)
integer       :: reg(npt),reg1(npt1)
integer       :: sk1(mxh3,mxh2,mxh1),ll1(npt1),mxh1,mxh2,mxh3,npt,npt1,nsepp,nsepv,nreg
integer       :: nt,nthr,fracp,dpart,nadv,nc1,nc2,nc3,jq1m,jq2m
integer       :: i,ii,j,jj,iq1,iq2,iq3,jq1,jq2,jq3,jq2min,jq2max,jq2t,p1,p2
integer       :: regi,regj
character     :: cntid*32,logf*80,progressf*512
...
!$omp& private(iq1,iq2,iq3,jq1,jq2,jq3,rcl,stm2,dltdec,jq1m) &
!$omp& private(dltra,jq2m,jq2min,jq2max,jq2t,j,i,xi,dci,rv,rp2,ii,jj,regi,regj) &
!$omp& schedule(guided) firstprivate(fracp) if(nthr>1)
do i=1,npt
   xi  = x(i)
   dci = dc(i)
...
subroutine rppi_Cjk_wg(nt,npt,ra,dec,dc,wei,x,y,z,reg,npt1,dc1,wei1,x1,y1,z1,reg1, &
           nreg,nsepp,sepp,nsepv,sepv,sbound,mxh1,mxh2,mxh3,wfib,cntid,logf,progressf,sk1,ll1,cdpv,touch)
implicit none
real(kind=8)  :: ra(npt),dec(npt),sbound(6),ral,rau,decl,decu,dcoml,dcomu,hc1,hc2,hc3,rp2
real(kind=8)  :: x(npt),y(npt),z(npt),x1(npt1),y1(npt1),z1(npt1),rcl,stm2,dltdec,dltra
real(kind=8)  :: dc(npt),dc1(npt1),xi,dci
...
real(kind=8)  :: cdpv(nsepv,nsepp),touch(nreg,nsepv,nsepp)
integer       :: reg(npt),reg1(npt1)
integer       :: sk1(mxh3,mxh2,mxh1),ll1(npt1),mxh1,mxh2,mxh3,npt,npt1,nsepp,nsepv,nreg,wfib
integer       :: nt,nthr,fracp,dpart,nadv,nc1,nc2,nc3,jq1m,jq2m
integer       :: i,ii,j,jj,iq1,iq2,iq3,jq1,jq2,jq3,jq2min,jq2max,jq2t,p1,p2
integer       :: regi,regj
character     :: cntid*32,logf*80,progressf*512
...
!$omp parallel do reduction(+:cdpv,touch) default(shared) &
!$omp& private(iq1,iq2,iq3,jq1,jq2,jq3,rcl,stm2,dltdec,jq1m) &
!$omp& private(dltra,jq2m,jq2min,jq2max,jq2t,j,i,xi,wi,dci,rv,rp2,shth2,wpp,ii,jj,regi,regj) &
!$omp& schedule(guided) firstprivate(fracp) if(nthr>1)
do i=1,npt
   xi  = x(i)
   dci = dc(i)
